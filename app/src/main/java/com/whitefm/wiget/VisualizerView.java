...
public class VisualizerView extends View implements Visualizer.OnDataCaptureListener {
	...
	private boolean isNeedShadows = true;

	public boolean isNeedShadows() {
		return isNeedShadows;
	}

	public void setNeedShadows(boolean needShadows) {
		isNeedShadows = needShadows;
	}

	//绘制频谱块和倒影
	protected void drawCylinder(Canvas canvas, float x, byte value) {
		if (value == 0&&isNeedShadows) {
			value = 1;
		}
		//最少有一个频谱块
		for (int i = 0; i < value; i++) { //每个能量柱绘制value个能量块
			if (isNeedShadows) {
				...
				float y = (getHeight()*5/8 - i * vgap - vgap);//计算y轴坐标
				//绘制频谱块
				mPaint.setColor(Color.WHITE);//画笔颜色
				canvas.drawLine(x, y, (x + strokeLength), y, mPaint);//绘制频谱块
			}
	...
	/**
	 * It sets the visualizer of the view. DO set the viaulizer to null when
	 * exit the program.
	 *
